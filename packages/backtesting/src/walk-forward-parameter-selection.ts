...
export interface WalkForwardParameterSelectionWindow extends WalkForwardWindow { selection: ParameterStabilityReport; test: BacktestPipelineResult; }
export interface WalkForwardParameterSelectionResult { windows: WalkForwardParameterSelectionWindow[]; outOfSample: PerformanceMetrics; }
...
export function runWalkForwardParameterSelection(input: WalkForwardParameterSelectionInput): WalkForwardParameterSelectionResult {
  ...
  const windows = createWalkForwardWindows(input.candles.length, input.walkForward).map((window) => {
    ...
    if (requireStableSelection && !selection.stable) throw new Error(`unstable parameter selection for walk-forward window ${window.trainStart}-${window.testEnd}: score spread ${selection.scoreSpreadPct.toFixed(2)}% exceeds the allowed threshold or best score is non-positive`);
    const strategyConfig = input.execution === undefined ? { quantity: input.quantity, strategy: selection.best.candidate.strategy } : { quantity: input.quantity, strategy: selection.best.candidate.strategy, execution: input.execution };
    const testResult = runBacktestPipeline({ candles: testCandles, initialCapital: input.initialCapital, strategy: strategyConfig, stressScenarios: input.stressScenarios, robustnessThresholds: input.robustnessThresholds });
