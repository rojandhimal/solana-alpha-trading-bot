...
const candles = Array.from({ length: 60 }, (_, i) => { const close = 100 + i * 0.5; return { open: close, high: close + 1, low: close - 1, close, volume: 100 + i }; });
const base = { fastPeriod: 5, slowPeriod: 10, rsiPeriod: 5, momentumPeriod: 5, atrPeriod: 5, volumePeriod: 5, entryThreshold: 0.5 };
const thresholds = { minPassingScenarioRatePct: 0, maxDrawdownPct: 100, minProfitFactor: 0, minExpectancy: -1_000 };

describe("parameter stability analysis", () => {
  it("normalizes expectancy into percentage-point units before scoring", () => {
    const metrics = { totalReturnPct: 12, netProfit: 1_200, maxDrawdownPct: 4, tradeCount: 10, winRatePct: 60, profitFactor: 1.5, expectancy: 100, averageWin: 200, averageLoss: 50 };
  ...
  it("rejects an empty candidate set and invalid stability thresholds", () => {
    expect(() => analyzeParameterStability({ candles, initialCapital: 10_000, quantity: 1, candidates: [], stressScenarios: [], robustnessThresholds: thresholds })).toThrow("at least one strategy candidate is required");
    expect(() => analyzeParameterStability({ candles, initialCapital: 10_000, quantity: 1, candidates: [{ strategy: base }], stabilitySpreadThresholdPct: -1, stressScenarios: [], robustnessThresholds: thresholds })).toThrow("stabilitySpreadThresholdPct must be non-negative");
  });
});
