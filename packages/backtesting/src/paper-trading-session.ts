...
export function runPaperTradingSession(candles: readonly StrategyCandle[], config: PaperTradingSessionConfig): PaperTradingSessionResult {
  ...
  const accounting = config.allowShort === undefined
    ? accountFills(candles, fills, config.initialCapital)
    : accountFills(candles, fills, config.initialCapital, { allowShort: config.allowShort });
  ...
  for (const point of accounting.equityCurve) {
    ...
    const positionNotionalPct = accounting.initialCapital === 0 ? 0 : Math.abs(point.positionValue) / accounting.initialCapital * 100;
