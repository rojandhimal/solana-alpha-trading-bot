...
export class PaperTradingState {
  ...
  constructor(config: PaperTradingStateConfig) {
    ...
    this.config = config; this.riskLimits = riskLimits;
  }
  ...
  snapshot(): PaperTradingSnapshot { const accounting = accountingFor(this.candles, this.fills, this.config.initialCapital, this.config.allowShort); const risk = riskFor(accounting, this.riskLimits); return { candleCount: this.candles.length, fillCount: this.fills.length, accounting, risk: { ...risk, halted: this.halted || risk.halted } }; }
